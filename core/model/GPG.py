...
from .discriminator import FCDiscriminator_O, FCDiscriminator_F
...
import sys
sys.path.append("..")
from utils.loss import Focal_CrossEntropy2d, BinaryCE, DiceLoss
...
class GPGNet(nn.Module):
    def __init__(self, cfg, stage='src'):
        super(GPGNet, self).__init__()
    ...
    def model_init(self, cfg):
        ...
        prototype_memory_s = torch.Tensor(1, inner_channel, nodes_num)
        self.register_buffer('prototype_memory_s', prototype_memory_s)
        prototype_memory_t = torch.Tensor(1, inner_channel, nodes_num)
        self.register_buffer('prototype_memory_t', prototype_memory_t)
        ...
        self.dice_criterion = DiceLoss(ignore_index=[0], class_num=cfg.NUM_CLASSES, ignore_label=255)
        img_size_t = cfg.DATASET.INPUT_SIZE_TARGET
        self.interp_t = nn.Upsample(size=(img_size_t[1], img_size_t[0]),
                                mode='bilinear', align_corners=True)

        if self.stage == 'adv':
            self.discriminator = FCDiscriminator_O(cfg.NUM_CLASSES)
            self.discriminator_aux = FCDiscriminator_O(cfg.NUM_CLASSES)
    ...
    def memory_updata(self, a, cfg):
        self.prototype_memory_s.data = self.intra_graph.multi_proto_t.data
        self.prototype_memory_t.data = self.intra_graph.multi_proto_s.data
    ...
    def src_step(self, a, i, image, label, cfg):
        ...
        loss_dice = self.dice_criterion(coarse_pred, label)
        loss = loss_c + loss_dice
    ...
    def adv_step(self, a, i, image, label, image_t, pseudo_label, relation_s, relation_t, cfg):
        ...
        x = self.backbone(image)
        x, x2 = self.intra_graph(x, 0) # intra graph
        ###
        ...
        ###
        coarse_pred = self.classifier(x)
        refine_x = self.inter_graph(x, coarse_pred.detach(), self.prototype_memory_s.detach(), relation_s) # inter graph
        refine_pred = self.ref_classifier(refine_x)
        refine_pred = self.interp(refine_pred)
        loss_r = self.cross_criterion(refine_pred, label)
        coarse_pred = self.interp(coarse_pred)
        loss_c = self.cross_criterion(coarse_pred, label)
        loss_dice_c = self.dice_criterion(coarse_pred, label)
        loss_dice = self.dice_criterion(refine_pred, label)

        loss_s = (loss_c + loss_aux + loss_dice_c) * cfg.TRAIN.AUX_WEIGHT + loss_r + loss_dice
        loss_s.backward()

        # adversarial process
        x_t = self.backbone(image_t)
        x_t, _ = self.intra_graph(x_t, 1) # intra graph
        coarse_pred_t = self.classifier(x_t)
        refine_x_t = self.inter_graph(x_t, coarse_pred_t.detach(), self.prototype_memory_t.detach(), relation_t)
        ...
        loss_adv = self.bce_criterion(d_pred_t, source_label)
        #loss_adv= self.bce_criterion(d_pred_t, source_label)
        #loss_adv_aux = self.bce_criterion(d_pred_aux_t, source_label)
        loss_adv_aux = self.bce_criterion(d_pred_aux_t, source_label)
        ...
        return (loss_s.item(), loss_adv_aux.item(), loss_dis_aux.item(), loss_dis_aux_t.item())

    def eval_bstep(self, i, image, label, cfg):
        x = self.backbone(image)
        coarse_pred = self.classifier(x)
    ...
    def eval_step(self, i, image, label, relation, cfg):
        x = self.backbone(image)
        x, _ = self.intra_graph(x, 1)
        coarse_pred = self.classifier(x)
        refine_x = self.inter_graph(x, coarse_pred, self.prototype_memory_t, relation)
        refine_pred = self.ref_classifier(refine_x)
    ...
    def relation_cal(self, image, label, d_flag, cfg):
        x = self.backbone(image)
        relation = self.intra_graph.relation_cal(x, label, d_flag, cfg)
