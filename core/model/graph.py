...
class Intra_graph(nn.Module):
    def __init__(self, in_channel, inner_channel, nodes_num, em_num, momentum):
        ...
        multi_proto_s = torch.Tensor(1, self.inner_channel, self.nodes_num)
        self.register_buffer('multi_proto_s', multi_proto_s)
        self.multi_proto_s.data.normal_(0, 0.001)
        pi_s = torch.ones([1, self.nodes_num])
        self.register_buffer('pi_s', pi_s)
        self.pi_s = self.pi_s / self.nodes_num

        multi_proto_t = torch.Tensor(1, self.inner_channel, self.nodes_num)
        self.register_buffer('multi_proto_t', multi_proto_t)
        self.multi_proto_t.data.normal_(0, 0.001)
        pi_t = torch.ones([1, self.nodes_num])
        self.register_buffer('pi_t', pi_t)
        self.pi_t = self.pi_t / self.nodes_num
    ...
    def forward(self, x, d_flag):
        ...
        with torch.no_grad():
            if d_flag == 0: # s0 t1
                mu = self.multi_proto_s.repeat(b, 1, 1).data
                pi = self.pi_s.repeat(b, 1, 1).data
            elif d_flag == 1:
                mu = self.multi_proto_t.repeat(b, 1, 1).data
                pi = self.pi_t.repeat(b, 1, 1).data
            ...
            if d_flag == 0: # s0 t1
                self.multi_proto_s.data = self.momentum * self.multi_proto_s.data + (1 - self.momentum) * mu.mean(0)
                self.pi_s.data = self.momentum * self.pi_s.data + (1 - self.momentum) * pi.mean(0)
            elif d_flag == 1:
                self.multi_proto_t.data = self.momentum * self.multi_proto_t.data + (1 - self.momentum) * mu.mean(0)
                self.pi_t.data = self.momentum * self.pi_t.data + (1 - self.momentum) * pi.mean(0)
    ...
    def relation_cal(self, x, label, d_flag, cfg):
        ...
        if d_flag == 0:
            mu = self.multi_proto_s.repeat(b, 1, 1).data
            pi = self.pi_s.repeat(b, 1, 1)
        elif d_flag == 1:
            mu = self.multi_proto_t.repeat(b, 1, 1).data
            pi = self.pi_t.repeat(b, 1, 1)
        for i in range(self.em_num):
            likelihood = torch.bmm(x1.transpose(1, 2), mu)
            likelihood = likelihood - torch.max(likelihood, 2, keepdim=True)[0].detach()
            likelihood = torch.exp(likelihood)
...
class Inter_graph(nn.Module):
    ...
    def forward(self, feature, coarse_pred, global_proto, relation):
        ...
        rand_relation = torch.rand(relation.shape).cuda(0)
        relation = (rand_relation <= relation).float()
