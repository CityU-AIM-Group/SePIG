...
def proto2center(model, cfg):
    ...
    for i, batch in enumerate(source_loader):
        with torch.no_grad():
            ...
            single_relation = model.relation_cal(image.cuda(cfg.GPU_ID), label.long().cuda(cfg.GPU_ID), 0, cfg)
    ...
    for i, batch in enumerate(target_loader):
        with torch.no_grad():
            image, label, _, name = batch
            single_relation = model.relation_cal(image.cuda(cfg.GPU_ID), label.long().cuda(cfg.GPU_ID), 1, cfg)
            for j in range(cfg.NUM_CLASSES):
                if np.sum(single_relation[j]):
                    count_t[j] += 1
